...
*Setup instructions for a Gitian build of XEYRA Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the XEYRA
...
- [Building XEYRA Core](#building-xeyra-core)
...
Clone the git repositories for xeyra and Gitian.
...
in the xeyra repository under 'Fetch and create inputs' to install sources which require
...
To build XEYRA Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the xeyra repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/xeyra/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/xeyra-project/xeyra
...
For example:
```bash
URL=https://github.com/thrasher-/xeyra.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit xeyra=${COMMIT} --url xeyra=${URL} ../xeyra/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit xeyra=${COMMIT} --url xeyra=${URL} ../xeyra/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit xeyra=${COMMIT} --url xeyra=${URL} ../xeyra/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the xeyra git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../xeyra/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/xeyra
SIGPATH=/some/root/path/xeyra-detached-sigs

./bin/gbuild --url xeyra=${BTCPATH},signature=${SIGPATH} ../xeyra/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/xeyra-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/xeyra-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/xeyra-osx-build.assert
```
...
[xeyra-project/gitian.sigs.xyr](https://github.com/xeyra-project/gitian.sigs.xyr/) repository, or if that's not possible create a pull
